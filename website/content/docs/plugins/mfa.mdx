...
| `secret_data` | `text` | Encrypted (Rails Active Record Encryption) |
...
- **TOTP secrets** are stored using Rails Active Record Encryption
- **Backup code digests** are stored as SHA-256 hashes; plain-text codes are shown once
- **SMS codes** are stored as digests with a 5-minute expiry
- TOTP verification allows a 30-second drift to accommodate clock differences
- Backup codes are consumed on use (one-time only)
